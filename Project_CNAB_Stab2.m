...
for i = 1:length(alpha)
    for j = 1:length(beta)
        ...
        B = I + alpha(i)/2*Dm(:,:,2) - 3*beta(j)/2*Dm(:,:,1);
...
for i = 1:length(alpha)
    for j = 1:length(beta)
        A = I - alpha(i)/2*Dspec*Dspec;
        A(end-1,:) = ones(1,N+1); A(end,:) = (-ones(1,N+1)).^(0:N);
        A = inv(A);
        
        B = I + alpha(i)/2*Dspec*Dspec - 3*beta(j)/2*Dspec;
...
set(gca,'xscale','log','yscale','log','XTick',[1e-7 1e-5 1e-3 1e-1],'YTick',[],'FontSize',20,'Linewidth',2)
hold off